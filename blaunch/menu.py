...
class Node:
	...
	def Match(self, path):	
		"""Return all possible matches for [path]. All returned matches will be
		siblings.
	...
	def Load(file_contents):
		"""Returns a root node with loaded hierarchy from the provided
		configuration file.
	...
	def _Parse(file_contents):
		"""Returns a list of dictionaries extracted from [file_contents].
	...
	def _Generate_Nodes(dictionaries):
		"""Returns a list of Nodes from the provided list of dictionaries.
	...
	@staticmethod
	def _Link(nodes):
		"""Returns a root node with a linked hierarchy.
